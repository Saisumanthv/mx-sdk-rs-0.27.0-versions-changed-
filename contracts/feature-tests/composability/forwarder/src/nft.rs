...
pub trait ForwarderNftModule: storage::ForwarderStorageModule {
    ...
    fn buy_nft(
        ...
        let payment_nonce = self.call_value().dct_token_nonce();
    ...
    fn nft_create(
        ...
        uris.push(uri);
        let token_nonce = self.send().dct_nft_create::<Color>(
            &token_identifier,
            &amount,
            &name,
            &royalties,
    ...
    fn nft_decode_complex_attributes(
        ...
        let mut uris = ManagedVec::new();
        uris.push(uri);
        let token_nonce = self.send().dct_nft_create::<ComplexAttributes<Self::Api>>(
            &token_identifier,
            &amount,
            &name,
            &royalties,
